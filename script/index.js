  ...
  data.forEach(function (elemento) {
    ...
    if (day==systemDay && mounth==systemMounth && year==systemYear){
      if (hour>=systemHour){
        ...
        // Crear el contenedor de información del usuario  de la publicación
        ...
        let promesaUsuario= fetch(`http://localhost:3000/users/${elemento.creatorID}`)
          ...
          .then(dataUsuario =>{
            ...
            ownerUsername.innerText = `@${dataUsuario.name}`;
    
            let followersCount = document.createElement('p');
            followersCount.innerText = `${dataUsuario.follower_counter} followers`;
    
            boxOwnerEventInfo.appendChild(ownerUsername);
            boxOwnerEventInfo.appendChild(followersCount);
    
            boxContainerPostInfo.appendChild(profileImg);
            boxContainerPostInfo.appendChild(boxOwnerEventInfo);
        ...
        eventType.innerText = 'Event';

        let category = document.createElement('p');
        category.innerText = elemento.category;

        let hashtags = document.createElement('span');
        hashtags.innerText = '#music, #street, #atnight';
        ...
        descriptionText.innerText = elemento.description;
        ...
        likesLink.href = '#';
        likesLink.innerText = ` ${elemento.likes} likes`;//likes



        let commentsLink = document.createElement('a');
        commentsLink.href = '#';
        commentsLink.innerText = ` ${elemento.comments} comments`;//comments
